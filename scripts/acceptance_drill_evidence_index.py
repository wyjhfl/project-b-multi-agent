...
DEFAULT_INPUT_DIR = ROOT_DIR / "docs" / "reports" / "controlled_production_acceptance"
DEFAULT_OUTPUT_DIR = ROOT_DIR / "docs" / "reports" / "acceptance_drill_index"
...
BOUNDARY_DECLARATIONS = [
    "只读 acceptance drill evidence index",
    "仅扫描受控生产验收演练 JSON 报告",
    "不读取 Markdown 报告正文",
    "不读取或输出真实 secret 原文",
    "不修改、不移动、不删除输入证据",
...
def _report_row(path: Path, payload: dict[str, Any]) -> dict[str, Any]:
    stat = path.stat()
    domains = payload.get("acceptance_domains") if isinstance(payload.get("acceptance_domains"), list) else []
    domain_status_counts: dict[str, int] = {}
    for item in domains:
        if isinstance(item, dict):
            status = _sanitize_text(item.get("status") or "unknown")
    ...
    return {
        ...
        "domain_count": int(payload.get("domain_count") or len(domains)),
        "review_ready_domain_count": int(payload.get("review_ready_domain_count") or 0),
        "domain_status_counts": domain_status_counts,
        "unexpected_execution_flags": [flag for flag in UNEXPECTED_FLAGS if bool(payload.get(flag, False))],
    }
...
def _derive_status(rows: list[dict[str, Any]], warnings: list[str]) -> str:
    ...
    if not rows:
        return "skipped"
    if any(not row.get("read_only") for row in rows):
        return "blocked"
...
def _build_markdown(payload: dict[str, Any]) -> str:
    ...
    for item in payload.get("reports", []):
        lines.append(
            f"- {item.get('path')}: {item.get('status')} | domains={item.get('domain_count', 0)} | review_ready={item.get('review_ready_domain_count', 0)}"
        )
...
def build_acceptance_drill_evidence_index(
    *,
    ...
    if not source_root.exists():
        ...
        for path in sorted(source_root.glob("*_controlled_production_acceptance_drill.json")):
            payload, load_warnings = _load_report(path)
            warnings.extend(load_warnings)
            if payload is not None:
                rows.append(_report_row(path, payload))

    rows.sort(key=lambda item: str(item.get("modified_at", "")), reverse=True)
    ...
    payload = {
        ...
        "totals": {
            "domain_count": sum(int(row.get("domain_count", 0)) for row in rows),
            "review_ready_domain_count": sum(int(row.get("review_ready_domain_count", 0)) for row in rows),
        },
        ...
        "go_no_go": {
            ...
            "auto_changed": False,
            "reason": "acceptance drill evidence index 只读汇总验收演练报告，不自动批准上线。",
        },
        "boundary_declarations": BOUNDARY_DECLARATIONS,
    }

    stem = f"{generated_at.replace(':', '-').replace('+', '_')}_{short_commit}_acceptance_drill_evidence_index"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="生成 v4.2 acceptance drill 只读索引（JSON + Markdown）")
...
def main() -> int:
    ...
    summary = build_acceptance_drill_evidence_index(output_dir=args.output_dir, input_dir=args.input_dir)
