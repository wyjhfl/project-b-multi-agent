...
def test_production_landing_action_pack_lists_remaining_real_inputs(tmp_path: Path, monkeypatch) -> None:
    ...
    assert payload["recommended_commands"][0] == (
        "powershell -NoProfile -ExecutionPolicy Bypass -File scripts\\codex_python.ps1 "
        "scripts\\production_landing_status.py"
    )
    assert payload["recommended_commands"][1].endswith("scripts\\production_landing_env_init.py")
    assert payload["recommended_commands"][2].endswith("scripts\\production_landing_local_infra_bootstrap.py")
    assert payload["recommended_commands"][3].endswith("scripts\\production_landing_env_template.py")
    assert (
        payload["recommended_commands"][4]
        == (
            "powershell -NoProfile -ExecutionPolicy Bypass -File scripts\\codex_python.ps1 "
            "scripts\\production_landing_env_runner.py --action real-llm-preflight"
        )
    )
    assert payload["recommended_commands"][5] == "powershell -NoProfile -ExecutionPolicy Bypass -File scripts\\real_llm_preflight.ps1"
    assert not any("--base-url http://" in command or "--base-url https://" in command for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\production_landing_local_mcp_bootstrap.py") for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\production_landing_local_business_bootstrap.py") for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\production_landing_env_template.py") for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\production_landing_env_check.py") for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\production_landing_execution_gate.py") for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\production_landing_status.py") for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\production_landing_env_runner.py --action env-check") for command in payload["recommended_commands"])
    assert any(
        command.endswith("scripts\\production_landing_env_runner.py --action real-llm-preflight")
        for command in payload["recommended_commands"]
    )
    assert any(command.endswith("scripts\\production_landing_env_runner.py --action staging-smoke") for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\production_landing_env_runner.py --action business-smoke") for command in payload["recommended_commands"])
    assert any("scripts\\controlled_pilot_demo_landing.ps1" in command for command in payload["recommended_commands"])
    assert any("scripts\\production_landing_closure_evidence_draft.py" in command for command in payload["recommended_commands"])
    assert not any(command.startswith("python scripts/") for command in payload["recommended_commands"])
    ...
    assert any(
        "scripts/production_landing_input_readiness.py" in _normalize_command(command)
        and "--closure-evidence" in command
        and _normalize_command(payload["resolved_paths"]["closure_evidence_draft"]) in _normalize_command(command)
        for command in payload["recommended_commands"]
    )
    ...
    assert infra_input["real_llm_preflight"]["api_key_present"] is False
    assert infra_input["real_llm_preflight"]["network_check_requested"] is True
    assert infra_input["real_llm_preflight"]["network_check_allowed"] is False
    assert "missing_process_env:XIAOMI_LLM_API_KEY" in infra_input["real_llm_acceptance_blockers"]
    assert infra_input["real_llm_safe_next_action"] == "run_scripts_xiaomi_llm_preflight_ps1_and_enter_key_securely"
    ...
    assert "REAL_LLM_MODEL=gpt-5.5" in infra_input["required_env"]
    assert "REAL_LLM_BASE_URL=http://100.119.206.22:8300/v1" in infra_input["required_env"]
    assert "REAL_LLM_API_KEY=<secret-managed-token>" in infra_input["required_env"]
    assert "DATABASE_URL=<secret-managed-url>" in infra_input["required_env"]
    assert "scripts\\real_llm_preflight.ps1" in infra_input["command_after_fill"]
    ...
    commands = "\n".join(payload["recommended_commands"])
    assert "scripts\\controlled_pilot_demo_landing.ps1" in commands
    assert "scripts\\business_system_read_smoke.ps1" in commands
    assert "scripts\\business_system_landing_resume.ps1 -UseExistingEnv -EnvPath local\\production_landing.staging.env" in commands
    ...
    assert any(command.endswith("scripts\\production_landing_text_quality_check.py") for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\manual_signoff_evidence_ack_status.py") for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\manual_signoff_record_draft.py") for command in payload["recommended_commands"])
    assert any("manual_signoff_record_promote.py" in command for command in payload["recommended_commands"])
    assert any(command.endswith("scripts\\manual_signoff_record_validator.py") for command in payload["recommended_commands"])
    ...
    promote_command = _normalize_command(manual_input["promote_command_after_manual_fill"])
    assert _normalize_command(payload["resolved_paths"]["manual_signoff_record_draft"]) in promote_command
    assert _normalize_command(payload["resolved_paths"]["manual_signoff_record"]) in promote_command
    ...
    assert any(command.endswith("scripts\\production_landing_xiaomi_llm_bootstrap.py") for command in payload["recommended_commands"])
    assert "powershell -NoProfile -ExecutionPolicy Bypass -File scripts\\xiaomi_llm_landing_resume.ps1" in payload[
    ...
    assert all(_normalize_command(command).startswith(allowed_prefixes) for command in payload["recommended_commands"])
...
def test_production_landing_action_pack_keeps_business_input_for_local_mock_success(
            ...
            "items": [
                {
                    ...
                    "recommended_accept": False,
                    "missing_conditions": ["business_read_smoke:local_business_mock_not_valid_for_real_production"],
                }
            ],
        },
    )
    ...
    _write_json(
        reports["launch_blocker_closure"],
        "001_launch_blocker_closure_workflow.json",
        {"status": "partial", "evidence_incomplete_count": 0},
    )

    summary = pack.build_production_landing_action_pack(output_dir=tmp_path / "out")
    payload = _read_payload(summary)
    business_input = next(
        item for item in payload["required_inputs"] if item["input_id"] == "business_system_read_only_credentials"
    )

    assert summary["status"] == "partial"
    assert business_input["reason"] == "local_business_mock_not_valid_for_real_production"
    assert business_input["command_after_fill"] == "powershell -NoProfile -ExecutionPolicy Bypass -File scripts\\business_system_read_smoke.ps1"
    assert payload["public_production_direct_launch"] == "No-Go"


def test_production_landing_action_pack_prefers_generic_real_llm_preflight_report(
    tmp_path: Path,
    monkeypatch,
) -> None:
    reports = {
    ...
    _write_json(
        reports["production_landing_real_llm_preflight"],
        "001_production_landing_real_llm_preflight.json",
        {
            "status": "success",
    ...
    _write_json(
        ...
        "001_production_landing_xiaomi_llm_preflight.json",
        {
            ...
            "preflight": {"network_check_requested": True, "network_check_allowed": False, "network_check_executed": False},
        },
    )
    _write_json(
        reports["production_pilot_signoff"],
        "001_production_pilot_signoff.json",
        {
            "status": "partial",
            "manual_signoff_completed": False,
            "landing_status": {"real_infra_ready": False, "production_blockers": ["real_infra:missing"]},
        },
    )
    ...
    _write_json(reports["real_integration_staging_smoke"], "001_real_integration_staging_smoke.json", {"status": "partial", "preflight_summary": {"domains": []}})
    ...
    assert infra_input["real_llm_preflight_source"] == "production_landing_real_llm_preflight"
    assert infra_input["real_llm_preflight"]["api_key_present"] is True
    assert infra_input["real_llm_acceptance_blockers"] == []
    assert infra_input["real_llm_safe_next_action"] == "refresh_landing_status_and_continue_manual_signoff"
    assert manual_input["blocking_evidence_items"][0]["acceptance_blockers"] == []
    assert manual_input["blocking_evidence_items"][0]["safe_next_action"] == "refresh_landing_status_and_continue_manual_signoff"


def test_production_landing_action_pack_does_not_request_infra_when_latest_smoke_succeeds(
    tmp_path: Path,
    monkeypatch,
) -> None:
    reports = {
    ...
    _write_json(reports["manual_signoff_package"], "001_manual_signoff_package.json", {"status": "partial"})
    _write_json(reports["closure_evidence_index"], "001_closure_evidence_index.json", {"status": "partial", "report_count": 1})
    _write_json(
        reports["launch_blocker_closure"],
        "001_launch_blocker_closure_workflow.json",
    ...
    assert "real_infra_current_round_acceptance" not in input_ids
