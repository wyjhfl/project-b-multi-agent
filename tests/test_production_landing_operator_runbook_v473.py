...
def test_production_landing_operator_runbook_documents_failure_and_safe_next_steps() -> None:
    ...
    assert "production_landing_real_llm_preflight_runner.py --execute-network-check --model gpt-5.5" in text
    assert "scripts\\xiaomi_llm_preflight.ps1" in text
    ...
    assert "controlled_pilot_demo_landing.ps1" in text
    ...
    assert "REAL_LLM_API_KEY" in text
