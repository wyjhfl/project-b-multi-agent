...
def test_operations_console_landing_smoke_prefers_generic_real_llm_preflight(
    tmp_path: Path,
    monkeypatch,
) -> None:
    payload = {
    ...
    assert summary["status"] == "success"
    assert report["checks"]["preflight_source"] == "production_landing_real_llm_preflight"
    assert report["checks"]["safe_next_action"] == "refresh_landing_status_and_continue_manual_signoff"
    assert report["checks"]["acceptance_blockers"] == []
    assert not any("xiaomi_preflight" in item for item in report["missing_conditions"])
...
def test_operations_console_landing_smoke_fails_when_expected_fields_missing(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "operations_summary:real_llm_preflight_network_check_requested_missing" in report["missing_conditions"]
    assert "operations_summary:real_llm_preflight_safe_next_action_missing" in report["missing_conditions"]
